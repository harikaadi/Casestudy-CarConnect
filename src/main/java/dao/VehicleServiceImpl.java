...
public class VehicleServiceImpl implements IVehicleService {
	 ...
	 private static final String query_update="UPDATE Vehicle SET Model=?, Make=?, year=?, colour=?, RegistrationNumber=?, Availability=?, DailyRate=? WHERE VehicleID=?";
   ...
   public boolean  getAvailableVehicles() {
        // Implementation code to get list of available vehicles from database
    	 try(Connection con=DBConnUtil.getConnection();Statement pstmt = con.createStatement()){
    		 ...
    		 while(rs.next())
    		 {
    			 System.out.println(" Vehicle " + rs.getInt("VehicleID") + ", Model: " + rs.getString("Model") + ", Make: " + rs.getString("Make")+" year "+rs.getString("year")+" colour "+rs.getString("colour")+" RegisteredNumber: "+rs.getString("RegistrationNumber")+" Availablity :"+rs.getInt("Availability")+" ,DailyRate "+rs.getInt("DailyRate"));
    		 }
